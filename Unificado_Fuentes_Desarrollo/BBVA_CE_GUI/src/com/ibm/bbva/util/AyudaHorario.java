...
public class AyudaHorario extends AbstractMBean {
	...
	private void actualizarDatos (long idOficina) {	
		...
		if(mapaDatosHorarioOficina!=null && mapaDatosHorarioOficina.size()>0){
			LOG.info("Tamanio mapaDatosHorarioOficina:::"+mapaDatosHorarioOficina.size());
			LOG.info("idOficina:::"+idOficina);
			lista = mapaDatosHorarioOficina.get(idOficina);
		}else
			LOG.info("mapaDatosHorarioOficina es null o vacío");
	...
	public int calcularMinutos (Calendar inicioTarea, Calendar ahora) {
		...
		LOG.info("inicioTarea:::"+inicioTarea.toString());
		LOG.info("ahora:::"+ahora.toString());
	...
	private class Secuencia {
        ...
        public int minutosMismoDia (Calendar fechaInicio, Calendar fechaFin) {
        	...
        	minInicio=minutosInicio (fechaFin);
        	minFin=minutosFin(fechaInicio);

        	result = minutosNor - (minutosEx==Constantes.NO_TIENE_MINUTOS ? 0 : minutosEx) - 
        			minInicio - minFin;
