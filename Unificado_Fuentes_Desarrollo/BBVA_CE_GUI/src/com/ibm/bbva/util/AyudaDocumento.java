	...
	public void adjuntarDocumentoExpediente() {
		...
		if (!(listGuiaDocumentaria == null || listGuiaDocumentaria.isEmpty())){
		    for (GuiaDocumentaria guia : listGuiaDocumentaria){
		    	...
		    	LOG.info("Gu�a: " +  guia.getId());
		    	if (guia.getId() <= 0) continue; // estos son documentos que se adjuntaron en tareas anteriores y solo se visualizan, no son parte de la gu�a para la tarea
