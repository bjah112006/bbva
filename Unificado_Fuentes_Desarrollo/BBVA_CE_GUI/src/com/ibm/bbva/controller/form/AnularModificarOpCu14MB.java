...
@ManagedBean(name = "anularModificarOpCu14")
@RequestScoped
public class AnularModificarOpCu14MB extends AbstractMBean {
	@EJB
	...
	private static final Logger LOG = LoggerFactory.getLogger(AnularModificarOpCu14MB.class);
	
	public AnularModificarOpCu14MB() {
		LOG.info("Entro AnularModificarOpCu14");
		...
		if (esValido()) {
			...
			if (documentosEscaneados.getValidaGuiaEscaneada().equals(Constantes.VALIDA_DOC_ESC_EXISTE)) {
				...
				if (objExpedienteDTO!=null){
				    ...
				    grabarExp(Constantes.ACCION_BOTON_CONFIRMAR_VALIDACION, Constantes.ESTADO_POR_REALIZAR_ALTA_TAREA_14);
				    addObjectSession(Constantes.FLAG_COPIA_ARCHIVO_SESION, Constantes.FLAG_COPIA_ARCHIVO);
	...
	/*public String accionVentana(List<DevolucionTarea> lista) {
		...
		grabarExp(Constantes.ACCION_BOTON_DEVOLVER, Constantes.ESTADO_EXPEDIENTE_OBSERVADO_TAREA_14);
