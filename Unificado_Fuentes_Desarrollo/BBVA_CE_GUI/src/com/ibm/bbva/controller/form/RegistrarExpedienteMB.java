...
//		}
		...
		LOG.info("gruposegmento Grabar : "+clienteNatural.getSegmento().getGrupoSegmento().getId());
		...
		if (esNuevo) {
			...
			if(clienteNatural.getEstadoCivil()!=null && clienteNatural.getEstadoCivil().getId()>0){
				if((clienteNatural.getEstadoCivil().getId()==Constantes.ESTADO_CIVIL_CASADO || 
					...
					LOG.info("clienteNaturalConyuge Grabar : "+clienteNaturalConyuge);
				    if (clienteNaturalConyuge!=null) {
				        LOG.info("clienteNaturalConyuge.getTipoCliente() Grabar : "+clienteNaturalConyuge.getTipoCliente());
				    }
				if (clienteNaturalConyuge != null) {
		...
		// para agilizar la reutilización de documentos se va a obviar la llamada al servicio de tipo de cambio al grabar pre-registro
		if (!Constantes.ACCION_BOTON_GRABAR.equals(accion)) {
		...
		if (Constantes.ACCION_BOTON_REGISTRAR.equals(accion)) { // accion aceptar
			...
			if (esNuevo) {
				...
				if("OK".equals(grabarReutilizable)){
					...
					LOG.info("expedienteTCWPS.getTaskID():::::"+expedienteTCWPS.getTaskID());
