...
public class PanelDocumentosMB extends AbstractMBean {
	 ...
	 * 
	 * Este atributo se modificar√° cada vez que se actualiza la lista
	 * de archivos del panel de documentos, sea por la agregaci√≥n de un documento
	 * o una eliminaci√≥n.
	 * 
    ...
    public void init() {		
		...
		/**
		 * Chequear si el expediente est· en proceso, en caso sea positivo
		 * el mensaje para todos los documentos del 'panel de documentos' ser·
		 * establecido como "En proceso"
		 * 
		 * El objeto ExpedienteTCWPS solo est· disponible cuando se realiza la llamada
	...
	*/
	...
	/**
	 * M√©todo que sucede luego de que el usuario
	 * presiona el bot√≥n de Aceptar, despu√©s de que se guarda el archivo a trav√©s
	 * del Applet, el cual llama a un m√©todo JavaScript que actualiza la propiedad
	...
	public void cargarDocumentosPanel(AjaxBehaviorEvent event){
		...
		LOG.info("øExisten nuevos archivos para el expediente en la tarea en curso? : " + this.isExistenNuevosArchivos());
		...
		/*} else if(productoNuevo.codSubproducto==null || productoNuevo.codSubproducto.equals(Constantes.CODIGO_CODIGO_CAMPO_VACIO) 
			...
			if(!getNombreJSPPrincipal().equals("formVisualizarExpediente") )
			 ...
			 if(productoNuevo!=null && (productoNuevo.codProducto==null || productoNuevo.codProducto.equals(Constantes.CODIGO_CODIGO_CAMPO_VACIO) 
					...
					if(!this.isOpValidar()){
						/*Documentos Escaneados*/
						LOG.info("Validaci√≥n de Documentos que ya fueron cargados");
						ELContext elContext = FacesContext.getCurrentInstance().getELContext();
						DocumentosEscaneadosMB documentosEscaneadosMB = (DocumentosEscaneadosMB) FacesContext
								.getCurrentInstance().getApplication().getELResolver()
								.getValue(elContext, null, "documentosEscaneados");
	...
	public void eliminarDocumentoContent(AjaxBehaviorEvent event){
		...
		try {
			...
			LOG.info("No se pudo eliminar el archivo en CM. ExcepciÛn.");
	...
	public void modificarDocumentoContent(AjaxBehaviorEvent event){
		...
		try {
			...
			if (!resultado.equals("KO")) { // ahora devuelve el PID
				 ...
				 * Al final en AyudaDocumento.java se crear√°n los que hagan falta 
				 * de nuevo. */
				if (docExpTCOld != null) {
			...
			LOG.info("No se pudo actualizar el archivo en CM. Excepci√≥n.");
			LOG.error(e.getMessage(), e);
		}
		cargarDocumentosPanel(event);
	}
	...
	public boolean validarDocumentos(){
		...
		if(this.isOpValidar()){
			// esta validaciÛn ya nunca se hace, opValidar siempre es false
...
//			}
						...
						if(!existeReutilizable){
							...
							if(existeGuia){
								...
								try {
									...
									LOG.info("No se pudo eliminar el archivo en CM. Excepci√≥n.");
									LOG.error(e.getMessage(), e);
								}
							}
						}
	...
	public List<PersonaPD> obtenerListaPanelDocumentoAgrupada(List<GuiaDocumentaria> listaGuiaDocAgr){
		...
		if(lstCodigoPersona!=null)
			LOG.info("lstCodigoPersona tamaÒo::::" +lstCodigoPersona.size());
		//Agrupaci√≥n por Persona.
		...
		// AgrupaciÛn por tipo de Documento.
		for (PersonaPD objPd : lstPersonaPDs) {			
			...
			//listTipoDocTmp=null; // no entiendo por quÈ ponÌan esta variable en null pero se cae si el for es de m·s de 1 por obvias razones
		...
		try{
			...
			if (objAyuda.getIdCm() != 0) {
				...
				if (lstDocExpAdj.size() >= 1) {
					...
					//Se obtiene la gu√≠a mas reciente.
					 ...
					 * cuando se hace clic en un expediente de la bandeja de histÛricos.
					 * 
					 * Se modificÛ el presente cÛdigo para capturar esa excepciÛn.
					 */
