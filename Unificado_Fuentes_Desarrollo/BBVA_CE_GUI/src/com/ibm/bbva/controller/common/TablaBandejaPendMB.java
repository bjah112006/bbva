...
public class TablaBandejaPendMB extends AbstractSortPagDataTableMBean {
	...
	public void init() {
        ...
        if(mapaDatosHorarioOficina==null){
    		...
    		if(mapaDatosHorarioOficina!=null)
    			LOG.info("Tama�o de mapa:::"+mapaDatosHorarioOficina.size());        	
        }
	...
	private void crearMapHorario (Map<Long, ArrayList<Horario>> mapa) {
		...
		if(mapa!=null){
			LOG.info("Tama�o de mapa:::"+mapa.size());
	...
	//	Empleado empleado = (Empleado) getObjectSession(Constantes.USUARIO_SESION);
		...
		if(lista!=null && lista.size()>0){
			...
			for (ExpedienteTCWPSWeb etc : lista){
				...
				//try{
					...
					if(etc.getIdOficina()!=null && !etc.getIdOficina().equals("")){
				        ...
				        if (nombJSP.equals("formBandejaBusqueda")) {
				        	LOG.info("band-busq --> etc.getCodigoUsuarioActual() : "+etc.getCodigoUsuarioActual());
					...
					/*if (etc.getFlagRetraer()!=null && etc.getFlagRetraer().equals(Constantes.FLAGRETRAERUNO)) {
						...
						//if(empleado != null && empleado.getPerfi<<l() != null){
							...
							//if(listTareaPerfil != null && listTareaPerfil.size() > 0){
								//LOG.info("Tama�o listTareaPerfil .. "+listTareaPerfil.size());
							//	listRetraccionTarea = retraccionTareaBean.buscarPorFlagRetraer(Long.parseLong(etc.getIdTarea()), listTareaPerfil.get(0).getTarea().getId());
								if(listRetraccionTarea!=null && listRetraccionTarea.size()>0){
									//LOG.info("Tama�o listRetraccionTarea ... "+listRetraccionTarea.size());
									...
									LOG.info("Tama�o listRetraccionTarea es nulo o vacio.. ");*/

								//LOG.info("RenderedRe..."+this.isRenderedRe());
							//}else
								//LOG.info("Tama�o listTareaPerfil nulo o vacio.. ");
						//}
					/*}*/ //else
		...
		if(listTabla!=null){
			this.setNumRegistros(String.valueOf(this.getListTabla().size())+" ");
			addObjectSession(Constantes.NUM_REG_BAND_PEND, String.valueOf(this.getListTabla().size()));
		}else{
			this.setNumRegistros("0 ");
			addObjectSession(Constantes.NUM_REG_BAND_PEND, "0 ");
		} 
		
		

		/**
		 * Ordenar Lista por C�digo de Expediente
		 * */
		if(listTabla!=null && listTabla.size()>0){
			
			LOG.info("Ordenando Lista por C�digo de Expediente");
				...
				if(!expedienteTC.getIdTarea().equals(TAREA_MANT_GEST_CM)){
					...
					if (getNombreJSPPrincipal().equals("formBandejaPendientes")){
						try{
							...
							LOG.error("No se encontr� la tarea " + expedienteTC.getIdTarea(), ex);
						}
						...
						if(expediente.getExpedienteTC().getTarea() != null){
							if(expediente.getExpedienteTC().getTarea().getId()!=Constantes.ID_TAREA_1 && expediente.getEstado().getId()!=Constantes.ESTADO_EN_REGISTRO_TAREA_1){
								...
								LOG.info("Fecha de activaci�n =========> " + fechaActivado.toString());
...
//		addObjectSession(Constantes.EXPEDIENTE_LISTA_DOCUMENTO_CM, mapListDocumentosCM);
	...
	public String retraer() {		
		 ...
		 * reestablecerlo. Esto se realizar� con la consulta
		 * al historial del expediente.
		 * 
		 * Guardamos el estado actual para luego guardarlo
		 * como parte del historial del expediente, este tendr�
		 * un flag de  RETRAER.
		 * 
		 */
        ...
        /** En el historial se guarda el expediente con el estado inicial que despu�s
         * fue cambiado al que ten�a anteriormente.
         */
		...
		try {
			Thread.sleep(1500); // esperar para que el expediente llegue a la bandeja de pendientes
		} catch(InterruptedException ex) {
			Thread.currentThread().interrupt();
		}
		
		// redirijo a bandeja de pendientes
		FacesContext ctx = FacesContext.getCurrentInstance();
		MenuMB menu = (MenuMB) ctx.getApplication().getVariableResolver().resolveVariable(ctx, "menu");
		return menu.bandejaPendientes();
	}
	...
	public void ordenar (ActionEvent event) {
		...
		if ("estado".equals(columna)) {
			...
			comparator = ComparatorFactory.nomUsuario(orden);
		} else if ("lineaCredito".equals(columna)) {
			comparator = ComparatorFactory.lineaCredito(orden);
		} else if ("montoAprobado".equals(columna)) {
			comparator = ComparatorFactory.montoAprobado(orden);
		...
		} else if ("codigoRVGL".equals(columna)) {
			comparator = ComparatorFactory.codigoRVGL(orden);
		} else if ("contrato".equals(columna)) {
			comparator = ComparatorFactory.contrato(orden);
		} else if ("observacion".equals(columna)) {
			comparator = ComparatorFactory.observacion(orden);
	...
	public void cargaUsuariosAsignar2(){
		...
		LOG.info("tama�o tabla ="+listTabla.size());
	...
	public String getNumRegistros() {
		numRegistros = (String) getObjectSession(Constantes.NUM_REG_BAND_PEND);
					...
					if(j<(inicial+ numReg)){  
				        if (nombJSP.equals("formBandejaBusqueda")) {
						      ...
						      if(obj.getExpedienteTC().getIdOficinaUsu()!=null && !obj.getExpedienteTC().getIdOficinaUsu().equals("")){
						        	if(obj.getAyudaHorario()==null){
						        		LOG.info("band-busq --> idExpediente : "+obj.getExpedienteTC().getCodigo());
						        		try {
					       			...
					       			LOG.info("IdOficinaUsu es vac�o");


						      /**
						       * Modificaci�n Flag Retraer - Visualizarlo
						       * 04 de Agosto 2015
						       * */
							 ...
							 if(obj.getExpedienteTC().getFlagRetraer()!=null && obj.getExpedienteTC().getFlagRetraer().equals("1"))
								 this.setRenderedRe(true);
						      /**
						       * Fin / Modificaci�n Flag Retraer - Visualizarlo
						       * */							 
