...
public class FileFTPService {
	...
	public static byte[] getContentFTPFile(String nomFile){
		...
		try {
			...
			logger.info("Codigo de respuesta de la conexion al FTP: "+cliente.getReplyCode());
			if (resultado) {
				retorno = os.toByteArray();
			} else {
				retorno = null;
				logger.info("El archivo "+nomFile+" no se pudo leer. No se enviará al content.");
			}
		} catch (Exception e) {
	...
	public static void autenticarCliente(FTPClient cliente) throws IOException {
		cliente.login(leerPropiedad(Constantes.FTP_USER),leerPropiedad(Constantes.FTP_PASS));
	...
	public static boolean renombrarFTPFile(String fromName, String toName) {
		...
		try {
			...
			if (blnChange) {
				...
				if (!resultado) {
					logger.info("Codigo de respuesta de la conexion al FTP: "+cliente.getReplyCode());
					logger.info("Cadena de respuesta de la conexion al FTP: "+cliente.getReplyString());
				}
			} else {
			...
			logger.error("Ocurrió un error al renombrar el archivo "+fromName, e);
			resultado = false;
		}
		return resultado;
	}
...
//		}
	...
	public static String leerPropiedad(String nombreProp) {
		...
		//return ResourceBundle.getBundle("/appContext").getString(nombreProp); //FIXME
