...
//@001A 10092015 Se implemento logica para el cobro de comision en base a las cuentas y a los poderes.
...
//			}
			...
			if (ConstantesBusiness.CODIGO_NUEVO_BASTANTEO.equals(expediente.getOperacion().getCodigoOperacion())
				...
				List<Cuenta> cuentas = identifiquePJOperacion1.getCuentas();
				cuentas = (cuentas == null) ? new ArrayList<Cuenta>() : cuentas;
				
				LOG.info("Cantidad Cuentas: " + cuentas.size());
				for(Cuenta c : cuentas){
					LOG.info("Cuenta: " + c.getNumeroContrato());
				}
				
				DatosClientePJSFP datosSFP = identifiquePJOperacion1.getPoderes();
				List<CuentaSFP> cuentasSFP = (datosSFP!=null && !datosSFP.getCuentasCorriente().isEmpty()) ? datosSFP.getCuentasCorriente() : new ArrayList<CuentaSFP>();
				LOG.info("Cantidad Cuentas SFP: " + cuentasSFP.size());
				for(CuentaSFP cSFP : cuentasSFP){
					LOG.info("CuentaSFP: " + cSFP.getNumeroCuenta());
				}
				
				//configurar los produtos de bastanteo
				boolean exonerado = verificarCuentasLibres(cuentas, cuentasSFP, expediente);
				LOG.info("Exconerado: " + exonerado);
				if(exonerado){
					
					expedienteCC.getDatosFlujoCtaCte().setClienteExonerado(String.valueOf(ConstantesBusiness.FLAG_EXONERADO_COBRO_COMISION));
					Util.addObjectSession(ConstantesAdmin.RESPUESTA_FLAG_COBRO_COMISION_SESION, "EXONERADO");
				}else{
					expedienteCC.getDatosFlujoCtaCte().setClienteExonerado(String.valueOf(ConstantesBusiness.FLAG_COBRO_COMISION));
					Util.addObjectSession(ConstantesAdmin.RESPUESTA_FLAG_COBRO_COMISION_SESION, null);
				}
			}
			//@001D018L
...
//			}
	...
	//@001A036L
	private boolean verificarCuentasLibres(List<Cuenta> cuentas,
			List<CuentaSFP> cuentasSFP, Expediente exp) {
		boolean findSFP = false;
		boolean resultado = false;
		for (Cuenta cuenta : cuentas) {
			findSFP = false;
			String numeroCuenta = cuenta.getNumeroContrato();
			LOG.info("verificarCuentasLibres:numeroCuenta: " + numeroCuenta);
			for (CuentaSFP cuentaSFP : cuentasSFP) {
				if (numeroCuenta.equalsIgnoreCase(cuentaSFP.getNumeroCuenta())) {
					LOG.info("verificarCuentasLibres:numeroCuenta encontrada: " + cuentaSFP.getNumeroCuenta());
					findSFP = true;
					break;
				}
			}
			if (findSFP == false) {
				if (exp.getProducto() != null) {
					MultiTabla multiTabla = null;
					try {
						multiTabla = multiTablaDAO
								.obtener(
										ConstantesBusiness.CODIGO_PADRE_PRODUCTO_EXONERADOS,
										exp.getProducto().getCodigo(),
										identifiquePJOperacion1
												.getCodigoSubProducto());
					} catch (NoResultException e) {
						LOG.info("Error multitabla", e);
					} catch (Exception e) {
						LOG.error("Error multitabla", e);
					...
					if (multiTabla != null) {
						LOG.info("verificarCuentasLibres:numeroCuenta proveedor segun multitabla " + numeroCuenta +"- SubProd : "+identifiquePJOperacion1.getCodigoSubProducto());
						resultado = true;
						break;
					} else {
						LOG.info("verificarCuentasLibres:numeroCuenta no proveedor segun multitabla: " + numeroCuenta +"- SubProd : "+identifiquePJOperacion1.getCodigoSubProducto());
						
					}
				}
			}else{
				LOG.info("verificarCuentasLibres:numeroCuenta sin registro de poderes en SFP" + numeroCuenta);
			}
