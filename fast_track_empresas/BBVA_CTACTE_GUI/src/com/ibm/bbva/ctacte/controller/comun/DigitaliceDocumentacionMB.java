	...
	private void crearListaDocumentoExp(String codTipoPJ, String codProd, Integer idCasoNegocio) {
		...
		if (expediente.getId() == null || esPreRegistroAutomaticoSinDocumentos()) {
			...
			for (DocumentoExp de : listaDocExp) {
				...
				// [Begin]-[15.07.23]-[Cambio valores de los documentos (casos de negocio)]
				try {
					arrayDocumentoCM = consultaCM.CM_historialDocumentoxCliente(de.getDocumento().getCodigoDocumento(), expediente.getCliente().getCodigoCentral(), ConstantesBusiness.NUM_DOC_HISTORICOS_RECIBIDOS_CM, de);
					...
					LOG.info("*********CATCH DigitaliceDocumentacion()********");
					e.printStackTrace();
					LOG.error("Consulta documentos CM :", e);
				...
				if (de.getDocumento().getFlagNroPagina().equals(Constantes.CHECK_SELECCIONADO) && arrayDocumentoCM != null && arrayDocumentoCM.length > 0) {
				...
				dew1.setNroPagina(nroPagina);
				
				Date fechaUltimoEscaneo = null;
				
				if (de.getDocumento().getFlagUltimaFecha().equals(Constantes.CHECK_SELECCIONADO) && arrayDocumentoCM != null && arrayDocumentoCM.length > 0) {
					DocumentoExp ultimoDocumentoHist = docExpDAO.findByIdCm(arrayDocumentoCM[0].getId());
					if (ultimoDocumentoHist != null) {
				...
				dew1.setUltimaFechaEscaneo(fechaUltimoEscaneo);
				// [End]-[15.07.23]
