			...
			if (subirArchivos) {
				...
				if (crearDocumentos) {
					...
					if (insBast!=null && insBast.length>0) {
						boolean resultado = Util.insertarDocumentoExp(ConstantesBusiness.CODIGO_DOCUMENTO_INSTRUCCIONES);
						if (resultado) {
					...
					if (dicBast!=null && dicBast.length>0) {
						boolean resultado = Util.insertarDocumentoExp(ConstantesBusiness.CODIGO_DOCUMENTO_DICTAMEN);
						if (resultado) {
							if (codConv!=null) {
								codConv += ";";
							}
