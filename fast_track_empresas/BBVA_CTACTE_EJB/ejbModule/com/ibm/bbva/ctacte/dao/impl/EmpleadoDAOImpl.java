...
public class EmpleadoDAOImpl extends GenericDAO<Empleado, Integer> implements EmpleadoDAO {
	...
	@Override
	public List<Empleado> getEmpleadosPorOficina(int idOficina) {
		Query query = em.createQuery(
 			"select o from Empleado o where o.oficina.id = :idOficina" );
		query.setParameter("idOficina", idOficina);
		List<Empleado> empleados = query.getResultList();
		return empleados;
	}
	...
	@Override
	public List<Empleado> getAllNotInLDAPTMP() {
		Query query = em.createQuery("select o from Empleado o where TRIM(o.codigo) NOT IN ( SELECT TRIM(e.codigoUsuario) from EmpleadoLdap e)");
		List<Empleado> empleados = query.getResultList();
		return empleados;
	}
	...
	@Override
	public List<Empleado> getAllInLDAPTMP() {
		Query query = em.createQuery("select o from Empleado o where TRIM(o.codigo) IN ( SELECT  TRIM(e.codigoUsuario) from EmpleadoLdap e)");
		List<Empleado> empleados = query.getResultList();
		return empleados;
	}
	...
	@Override
	public Empleado getByUserCode(String usuario) {
		Query query = em
				.createQuery("select o from Empleado o where o.codigo = :usuario");
	...
	@Override
	public List<Empleado> getGestorActivosPorOficina(int idOficina, String gestor) {
		Query query = em.createQuery(
 			"select o from Empleado o where o.oficina.id = :idOficina and o.flagActivo = '1' and UPPER(o.perfil.descripcion) like :nomPerfil " );
		query.setParameter("idOficina", idOficina);
		query.setParameter("nomPerfil", gestor.toUpperCase());
		List<Empleado> empleados = query.getResultList();
		return empleados;
	}
