...
public class HistorialDAOImpl extends GenericDAO<Historial, Integer> implements HistorialDAO {
	...
	@Override
	public boolean existeTareaExpedienteEnHistorial(Expediente e,
			int idTarea) {
		List<Historial> historial  = new ArrayList<Historial>();
		Query query = em.createQuery(
	 			"select o from Historial o where o.expediente.id=:idExp and o.tarea.id=:idTarea ");
			query.setParameter("idExp", e.getId());
			query.setParameter("idTarea", idTarea);
