...
public class ActualizarFechaInstancia implements RestApiController {
    ...
    RestApiResponse doHandle(HttpServletRequest request, PageResourceProvider pageResourceProvider, PageContext pageContext, RestApiResponseBuilder apiResponseBuilder, RestApiUtil restApiUtil) {
        ...
        try {
            ...
            String changedate = dfmt.parse(request.getParameter("changedate"))
            String rootprocessinstanceid = Long.parseLong(request.getParameter("rootprocessinstanceid"))
            Object[] params = [changedate, rootprocessinstanceid]
            
            SQLUtil<Object> sqlUtil = new SQLUtil<Object>("java:comp/env/bonitaSequenceManagerDS")
            sqlUtil.executeUpdate("UPDATE PROCESS_INSTANCE SET " + fielddate.toUpperCase() + "=? WHERE rootprocessinstanceid=?", params)
