...
public class TablaBandejaAsigMB extends AbstractSortPagDataTableMBean {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(TablaBandejaAsigMB.class);
	...
	public void actualizarLista() {
		...
		// //<<<-----+++----BORRAR----+++++---------+++++++++++++++++++
		if (lista != null && lista.size() > 0) {
			Iterator<ExpedienteTCWPS> iterador = lista.listIterator();
			while (iterador.hasNext()) {
				ExpedienteTCWPS expeTCWPS = (ExpedienteTCWPS) iterador.next();
				// Empleado empleadoVO =
	...
	public void actualizarUsuariosAsignacion() {
		...
		if(listaEmp!=null && listaEmp.size()>0){
			...
			/**
			 * 16/12/14
			 * Este código necesita un mejor
			 * algoritmo de recorrido para mejorar la 
			 * velocidad de respuesta al seleccionar todos
			 * los expedientes de una tabla.
			 */
	...
	private Expediente iniciarExpediente(String valor) {
		...
		if(expediente!=null){
			...
			if (getNombreJSPPrincipal().equals("formBandejaPendientes") || valor.equals("1")){
				...
				System.out.println("TAREA DESCRIP = "
						+ expediente.getExpedienteTC().getTarea().getDescripcion());
				System.out.println("ESTADO ID = " + expediente.getEstado().getId());
				System.out.println("ESTADO DESCRIP = "
				...
				if (expediente.getExpedienteTC().getTarea().getId() != Constantes.ID_TAREA_1
					...
					System.out.println("Actualizacion de TI y T2 ");
				...
				System.out.println("Num Doi 1 ="
						+ expediente.getClienteNatural().getNumDoi());
				System.out.println("Num Doi 2 ="
						+ objDocumentoExpTc.getExpediente().getClienteNatural()
								.getNumDoi());
