...
public class BuscarConyugeMB extends AbstractMBean {
	...
	public void init() {
		...
		/**
		 * @author Daniel Flores
		 * Antes se chequeaba si el getClienteNaturalConyuge de expediente en sesión era null y en base
		 * a eso se mostraba el panel de conyuge. Lo cual era incorrecto, porque aunque esté null debería
		 * mostrarlo en base al estado civil. Estos cambios solucionan esto. 
		 * 
		 * El punto es que al momento de depurar el código no se determina específicamente cómo funciona el flujo. 
		 * Al cambiar de estado soltero a casado, primero se llega a llamar esta sección de código con lo que datosClienteMB.isCasado
		 * retorna false designando mostrarBuscarConyuge como false también.
		 * 
		 * He revisado y mostrarBuscarConyuge no se utiliza en la vista, por lo tanto mostrarBuscarConyue = true no sirve
		 * de nada :). El que se encarga de mostrar esa panel de conyugue es una variable en RegistrarExpedienteMB de nombre
		 * visualizarPanelConyuge . Esta sección de código sirve para inyectar los datos en ese panel. 
		 * 
		 */
