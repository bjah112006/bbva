...
public class PanelDocumentosMB extends AbstractMBean {
    ...
    public void init() {		
		...
		ExpedienteTCWPS expedienteTCWPS = (ExpedienteTCWPS) getObjectSession(Constantes.EXPEDIENTE_PROCESO_SESION);
	...
	public void cargarDocumentosPanel(AjaxBehaviorEvent event){
		 ...
		 * Se realiza la revisión de la lista de documentos resultante,
		 * la comparativa también se realiza frente a 'null' porque por alguna razón todavía no analizada
		 * al momento de modificar este código, la variable contiene el valor nulo en su formato textual.
		 * 
		 * Actualización
		 * 28/01/14
		 * @author Daniel Flores
		 * Aquí se genera el error que en determinadas situaciones establecía el flagEnvioContent como "1"
		 * cuando no se habían adjuntado documentos. Mi peor split.
		 */
				...
				if (!personaAnt.equals("") && !ObligatorioAnt.equals("")){
					if(ldg.getPersona().getCodigo().trim().equals(personaAnt.trim())){
						...
						if(ldg.getObligatorio().trim().equals(ObligatorioAnt)){
							...
							System.out.println("ldg.getIdCm(): " + ldg.getIdCm());
							if(mapListDocumentosCM!=null)									
								objDocumento= (Documento) mapListDocumentosCM.get(String.valueOf(ldg.getIdCm()));
							
							System.out.println("tipo doc = "+ldg.getTipoDocumento().getCodigo());
							if(objDocumento!=null) {
	         					objAyuda.setUrlDocumento(objDocumento.getUrlContent().trim());
								System.out.println("url contentManager: "+objDocumento.getUrlContent());
								...
								objAyuda.setUrlDocumento("");
								objAyuda.setDocEscaneado(false);
								System.out.println("objDocumento es nulo");
							}								
			...
			for (TipoDocumentoPD objTipoDocPD : objPd.getLstTipoDocumentoPDs()) {				
				...
				for (GuiaDocumentaria ldg : listaGuiaDocumentosAgr) {
					if (objTipoDocPD.getTipoDocumento().getCodigo().equals(ldg.getTipoDocumento().getCodigo())) {
						...
						if(objDocumento!=null) {
         					objAyuda.setUrlDocumento(objDocumento.getUrlContent().trim());
         					LOG.info("url contentManager: "+objDocumento.getUrlContent());
