...
@ManagedBean(name = "consultarClienteModificaciones")
@RequestScoped
public class ConsultarClienteModificacionesMB extends AbstractMBean {
	...
	private static final Logger LOG = LoggerFactory.getLogger(ConsultarClienteModificacionesMB.class);
	
	public ConsultarClienteModificacionesMB() {
		
	}
	
	@PostConstruct
    public void init() {
		LOG.info("Entro ConsultarClienteModificaciones");
	...
	public String aceptar () {
		LOG.info("Actualizar Expediente - Exp : "+expediente.getId());
		...
		if (esValido()) {
			...
			if (documentosEscaneados.getValidaGuiaEscaneada().equals(Constantes.VALIDA_DOC_ESC_EXISTE)) {
				...
				if (expediente.getExpedienteTC().getFlagModifScore().equals("1")) {
					actualizar(Constantes.ACCION_BOTON_ACTUALIZAR_EXPEDIENTE, Constantes.ESTADO_CAMBIO_A_EN_TRAMITE_TAREA_16);	
				}else{
					actualizar(Constantes.ACCION_BOTON_ACTUALIZAR_EXPEDIENTE, Constantes.ESTADO_EN_VERIFICACIONES_TAREA_16);
				}
			}else{
			...
			addObjectSession(Constantes.TIPO_MENSAJE, Constantes.TIPO_MENSAJE_CONSULTAR_CLIENTE_MODIFICACIONES);
	...
	private String obtenerMensajeBD(String idMensaje, String idSesion) {
		...
		if (mensajeBD.trim().equals("")) {
			try {
				...
				mensajeBD = Mensajes.getMensaje("com.ibm.bbva.mensajes.formMensajes.consultarClienteModificaciones");
			}
		}		
		return mensajeBD;
	}
	...
	public String cancelarOperacion () {	
		...
		if (esValido()) {
			...
			if (!Constantes.CODIGO_PRESTAMO_FORMALIZADO_TAREA16.equals(expediente.getEstado().getCodigo())) {
				ELContext elContext = FacesContext.getCurrentInstance().getELContext();
	...
	public String accionVentana(ArrayList<DevolucionTarea> lista) {
		...
		actualizar(Constantes.ACCION_BOTON_RECHAZAR_EXPEDIENTE, Constantes.ESTADO_RECHAZADO_TAREA_16);
