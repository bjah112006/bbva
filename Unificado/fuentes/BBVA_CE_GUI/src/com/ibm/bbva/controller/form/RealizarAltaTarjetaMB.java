...
public class RealizarAltaTarjetaMB extends AbstractMBean {
	...
	private boolean esValido() {
		...
		//boolean validaNumContrato = validaNumContrato();
		/**
		 * Cambio James 28/04/14
		 * */
		//boolean validaNumContrato=true;
		boolean validaNumContrato=false;
		

		
		LOG.info("datProd : "+datProd);
		
		if(datosProducto3MB != null && datosProducto3MB.getProducto().getId() == Constantes.ID_APLICATIVO_TC){
			//validaNumContrato = validaNumContrato();
			
			if(expediente!=null && expediente.getExpedienteTC()!=null && expediente.getExpedienteTC().getNroContrato()!=null && 
					!expediente.getExpedienteTC().getNroContrato().trim().equals("")){
				LOG.info("Nro Contrato  : "+expediente.getExpedienteTC().getNroContrato());
				validaNumContrato=datosProducto3MB.validarNumeroContrato(expediente.getExpedienteTC().getNroContrato());
				
			}
			
		}else
			validaNumContrato=true;
		
		/*TODO
		 * VARIABLE validaNumContrato DEVUELVE False
		 * VARIABLE validaNumContrato SE ESTA COLOCANDO True PARA PRUEBAS
		 * */
		//validaNumContrato=true;
		
		msgErrorPersonalizado = datosProducto3MB.getMsgErrorPersonalizado();
		msgPersonalizado = datosProducto3MB.getMsgPersonalizado();
		return datProd && validoComentario && validaNumContrato;
	}	
