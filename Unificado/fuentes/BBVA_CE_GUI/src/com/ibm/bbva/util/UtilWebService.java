...
public class UtilWebService {
	...
	public static String validacionNumeroContradoClientePackPyme(BBVAFacadeLocal bbvaFacade, MensajesBeanLocal mensajeBean, Expediente expediente, String idEmpleado, String idSession){
		...
		if(strNumeroContrato != null ){
			...
			if(codigoRespuesta.equals(com.ibm.bbva.service.Constantes.PACKPYME_RESPUESTA_CONTRATO_ENCONTRADO)){ 
				validaNumContratoPackPyme = true;
			} else if(codigoRespuesta.equals(com.ibm.bbva.service.Constantes.PACKPYME_RESPUESTA_CONTRATO_NO_CORRESPONDE_CLIENTE)){
				idMensaje = Constantes.PACKPYME_RESPUESTA_CONTRATO_NO_CORRESPONDE_CLIENTE_MSG;
			} else if(codigoRespuesta.equals(com.ibm.bbva.service.Constantes.PACKPYME_RESPUESTA_CONTRATO_NO_ENCONTRADO)){
				idMensaje = Constantes.PACKPYME_RESPUESTA_CONTRATO_NO_ENCONTRADO_MSG;
			} else if(codigoRespuesta.equals(com.ibm.bbva.service.Constantes.PACKPYME_RESPUESTA_ERROR_ACCESO)){
				idMensaje = Constantes.PACKPYME_RESPUESTA_ERROR_ACCESO_MSG;
			} 
			
			if(!validaNumContratoPackPyme){
				...
				LOG.info("ErrorWS: " + codigoRespuesta + " al consultar el nro contrato: "+ expediente.getExpedienteTC().getNroContrato());			
			}
		} else{
