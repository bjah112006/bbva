	...
	public void run() {
		...
		while (!cola.isEmpty()) {
			...
			try {
				...
				boolean changeWDSuccess = fc.changeWorkingDirectory(ConstantesApplet.CARPETA_FTP);
				if (!changeWDSuccess) {
					LOG.error("Falló el cambio a la carpeta '" + ConstantesApplet.CARPETA_FTP + "'. Código de respuesta: "+fc.getReplyCode());
					reintentarEnvio(wrapper, cola);
					continue;
				}				
