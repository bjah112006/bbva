...
@ViewScoped
public class VerificarCalidadDocumentoMB extends AbstractMBean 
	implements IDatosGenerales, IDatosCliente, IResultadoRevision, IDocumentacionDigitalizada {

	private static final long serialVersionUID = 1L;
	
	private static final Logger LOG = LoggerFactory.getLogger(VerificarCalidadDocumentoMB.class);
	...
	private boolean faltaHistorialRequeridos;  //true si existe algun documento requerido q No tenga Historial 
												 //falso si todos los documentos requeridos tienen Historial
	...
	public void setErrorValidacionFechaVigencia(boolean errorValidacionFechaVigencia) {
	...
	public void setDocumentacionDigitalizada(DocumentacionDigitalizadaMB docDigitalizada) {
	...
	public void aplicarCambioTipoDocumento(List<DocumentoExpWrapper> listaDocumentoExpW){
		...
		for (int i = 0; i < listaDocumentoExpW.size(); i++) {
			listaDocumentoExp.add(((DocumentoExpWrapper)listaDocumentoExpW.get(i)).getDocExpediente());
		}
...
//+POR SOLICITUD BBVA+//+POR SOLICITUD BBVA+System.out..println("contador** -->"+cont);
			if (cont > 1){
				mensajeErrorPrincipal("idDocDigitalizada:tablaDocumentos", "Existen documentos duplicados");
...
//			docDigitalizada.actualizarEstadoBtnMerge();
	...
	public void aprobarDocumentacion(String accion){
		...
		//Realizo las validaciones en caso los flags de error estuvieran en true
	...
	public void actualizarBtnAprobarRechazar(boolean deshBtnAprobar, boolean deshBtnRechazar) {
		LOG.info("actualizarBtnAprobarRechazar({}, {})", deshBtnAprobar, deshBtnRechazar);
	...
	public void grabarInformacion(String accion){
		...
		List<DocumentoExp> listaDocExpediente = docDigitalizada.getListaDocExpediente();
		DocumentoExp docExp = null;
		...
		expediente.setObservacionesBastanteo(datosGenerales.getMensajePlazoSubsanacion());
		LOG.info("expediente.getObservacionesBastanteo(): " + expediente.getObservacionesBastanteo());
		expediente.setAccion(accion);
		//+POR SOLICITUD BBVA+//+POR SOLICITUD BBVA+System.out..println("estado Id "+expediente.getEstado().getId());
		//+POR SOLICITUD BBVA+//+POR SOLICITUD BBVA+System.out..println("estado Id "+expediente.getEstado().getDescripcion());
		//En este obj expediente se actualiza: Doc Historico, Fecha Vigencia, si aplica: Rechazo y Motivo, Observaciones 
		
		Tarea tarea = new Tarea();
		EstadoTarea estadoTarea = new EstadoTarea();
		
		//Genero un registro en Historial con los datos nuevo
		if(accion.equalsIgnoreCase(ConstantesBusiness.ACCION_RECHAZAR_DOCUMENTACION)){
		...
		}else if (accion.equalsIgnoreCase(ConstantesBusiness.ACCION_APROBAR_DOCUMENTACION)){
			//expediente.getEstado().setId(ConstantesBusiness.ID_ESTADO_EXPEDIENTE_ENCURSO);
...
//		redirectAction("../bandeja/bandeja");
	...
	public void renombrarArchivos(ActionEvent ae){
		...
		for (DocumentoExpWrapper dew : docDigitalizada.getListaDocExpedienteW()) {
			//+POR SOLICITUD BBVA+//+POR SOLICITUD BBVA+System.out..println("renombrarArchivos1 "+dew.isNewDocHistorial());
			if (dew.isNewDocHistorial() ){ // si el registro tiene un nuevo doc historial
				//+POR SOLICITUD BBVA+//+POR SOLICITUD BBVA+System.out..println("renombrarArchivos2");
				String nombreActual = dew.getNombreFile().concat(ConstantesAdmin.EXTENSION_ARCHIVO_PDF);
				String nombreNuevo = dew.getNombreFileFinal().concat(ConstantesAdmin.EXTENSION_ARCHIVO_PDF);
//				archivos.append("{\"actual\":\"").append(nombreActual).append("\",\"nuevo\":\"").append(nombreNuevo).append("\"}");
				archivos.append(nombreActual).append(":").append(nombreNuevo).append(";");
				//+POR SOLICITUD BBVA+//+POR SOLICITUD BBVA+System.out..println("archivos "+archivos.toString());
			}
		}
	...
	public void actualizarBtnAprobarRechazarbyErrores(){
		...
		LOG.info("errorValidacionFechaVigencia-->"+errorValidacionFechaVigencia);
		LOG.info("errorCambioTipoDocumento-->"+errorCambioTipoDocumento);
		LOG.info("existeRechazoSeleccionado-->"+existeRechazoSeleccionado);
		LOG.info("errorSeleccionarMotivoRechazo-->"+errorSeleccionarMotivoRechazo);
		LOG.info("faltaHistorialRequeridos-->"+faltaHistorialRequeridos);
		
		//Si es operacion Nuevo Bastanteo entonces anulación la validación Faltan Historialles para los Obligatorios
		...
		if (isExisteRechazoSeleccionado()){ // si hay rechazo 
			if (isErrorSeleccionarMotivoRechazo()){ // y no se seleccionó motivo de rechazo
				actualizarBtnAprobarRechazar(true,true);
				LOG.info("CASO 1");
			}else if (!isErrorCambioTipoDocumento() && !isErrorValidacionFechaVigencia()){// //y no hay otros errores
		...
		}else if (isErrorCambioTipoDocumento() || isErrorValidacionFechaVigencia() || isFaltaHistorialRequeridos() ){ 
			// si hay error en retipificacion o en la fecha de vigencia o falta historico de archivos requeridos
			actualizarBtnAprobarRechazar(true,true);
			LOG.info("CASO 4");
