...
public class DocumentacionDigitalizadaMB extends AbstractMBean {
	...
	public void iniciarSeccionDocumentacionDigitalizada(){
		...
		com.ibm.bbva.cm.service.Documento[] arrayDocumentoCM = null;
		...
		for (int i = 0; i < listaDocExpediente.size(); i++) {
			...
			try {
				...
				arrayDocumentoCM = consultaCM.CM_historialDocumentoxCliente(listaDocExpediente.get(i).getDocumento().getCodigoDocumento(), expediente.getCliente().getCodigoCentral(), ConstantesBusiness.NUM_DOC_HISTORICOS_RECIBIDOS_CM, listaDocExpediente.get(i));
			...
			LOG.info("listaDocExpediente.get(i).getFlagObligatorio()-->"+listaDocExpediente.get(i).getFlagObligatorio());
			if (listaDocExpediente.get(i).getFlagReqEscaneo().equals(ConstantesAdmin.FLAG_VERDADERO) && !existeArchivosHistoricos )
		...
		com.ibm.bbva.cm.service.Documento[] arrayDocHist = new com.ibm.bbva.cm.service.Documento[0];
...
//				faltaDocumentos = true;
				...
				if (dew1.isNewDocHistorial() != true && (dew1.getDocumentoHistCM() != null && dew1.getDocumentoHistCM()[0].getTipo().equalsIgnoreCase(ConstantesBusiness.CODIGO_DOCUMENTO_COPIA_LITERAL))) {
					/* Comentado porque causaba problemas si la copia literal no era el primer documento de la guía */
//					if (!esPrimero) {
//						descargaArcHist.append(";");
//					}
					descargaArcHist.append(dew1.getDocumentoHistCM()[0].getTipo()).append(";")
							       .append(dew1.getDocumentoHistCM()[0].getNombreArchivo()).append(";")
							       .append(dew1.getDocumentoHistCM()[0].getId());

					LOG.info("descargaArcHist " + dew1.getDocumentoHistCM()[0].getTipo()
							+ ";" + dew1.getDocumentoHistCM()[0].getNombreArchivo() 
							+";" + dew1.getDocumentoHistCM()[0].getId());
...
//		managedBean.actualizarSubidaDocumentos (faltaDocumentos);
	...
	public void actualizarDocumentos (ActionEvent ae) {
		...
		if (listaDocumentos!=null && !listaDocumentos.equals("")) {
			...
			com.ibm.bbva.cm.service.Documento[] arrayDocumentoHistorico;
			...
			while (st.hasMoreTokens()) {
				...
				if (doc!=null) {
					com.ibm.bbva.cm.service.Documento docHistorico = new com.ibm.bbva.cm.service.Documento();
					arrayDocumentoHistorico = new com.ibm.bbva.cm.service.Documento[1];
			...
			for (int j = 0; j < getDocumentoNewHistCM().length; j++) { 
				LOG.info("id del historico ["+j+"]-->"+ getDocumentoNewHistCM()[j].getId());
			...
			for (int i = 0; i < listaDocExpedienteW.size(); i++) {
				...
				for (int j = 0; j < listaDocExpedienteW.get(i).getDocumentoHistCM().length; j++) {
					...
					LOG.info("id Documento["+j+"]-->"+ listaDocExpedienteW.get(i).getDocumentoHistCM()[j].getId());
				}
			}
			//fin
	...
	public void eliminarDocumento (ActionEvent ae) {
		...
		com.ibm.bbva.cm.service.Documento[] docHistCM = new com.ibm.bbva.cm.service.Documento[0]; 
	 ...
	 private static class ArcHistCompare implements Comparator<com.ibm.bbva.cm.service.Documento>
	 {
	   public int compare(com.ibm.bbva.cm.service.Documento s1, com.ibm.bbva.cm.service.Documento s2)
	   {
