...
import com.ibm.bbva.ctacte.servicio.web.WEBCEServiceDelegate;
import com.ibm.bbva.ctacte.servicio.web.WEBCEServiceService;
...
public class AppApplet extends ArchivoApplet {
	...
	private synchronized void ejecutarAccion () {
		if (Accion.ABRIR_DOCUMENTO.equals(accion)) {
			...
			try {
				//+POR SOLICITUD BBVA+//+POR SOLICITUD BBVA+System.out..println("anter de ELIMINAR el archivo-->"+codigoDocumento);
				Archivo.getInstance().eliminarArchivo(codigoDocumento);
				ejecutarFuncion("confirmaEliminaDocumento", codigoDocumento);
			} catch (IOException e) {
...
//    	}
	...
	protected void findDocumento(Integer idExpediente, String codDocumento) throws Exception {
		//+POR SOLICITUD BBVA+//+POR SOLICITUD BBVA+System.out..println("findDocumento()");
		String url = "http://"+servidor+":"+puerto+"/BBVA_CTACTE_Servicio/WEBCEServiceService";
		//+POR SOLICITUD BBVA+//+POR SOLICITUD BBVA+System.out..println("url: "+url);
		WEBCEServiceService servicio = new WEBCEServiceService(new URL(url));
		WEBCEServiceDelegate port1 =   servicio.getWEBCEServicePort();		
		Documento documento = port1.cmObtenerDocumentoxCodigo(codDocumento, idExpediente);
		//+POR SOLICITUD BBVA+//+POR SOLICITUD BBVA+System.out..println(documento.getCodCliente());
		//Archivo.getInstance().guardarArchivo(codDocumento, documento.getContenido());
		Archivo.getInstance().descargarPDFContent(codDocumento, documento.getUrlContent());
	}
	...
	public void findDocumentoNombrar(Integer idExpediente, String codDocumento,String strNombreDoc) throws Exception {
		...
		Archivo.getInstance().descargarPDFContent(strNombreDoc, documento.getUrlContent());
	}
	...
	public void findDocumentoNombreArch(String tipoDocumento, String nombreArchivo, Integer idCM) throws Exception {
		...
		Archivo.getInstance().descargarPDFContent(tipoDocumento+"-HIST", documento.getUrlContent());
	}
	...
	public void ejecutarFuncionJS (String funcion, Object... parametros) {
		LOG.info("ejecutarFuncionJS (String funcion, Object... parametros)");
		try {
	...
	public void renombrarArchivosApp(String strCodigos) {		
		...
		//+POR SOLICITUD BBVA+//+POR SOLICITUD BBVA+System.out..println("--renombrarArchivosApp-strCodigos "+getStrCodigos());
	}
